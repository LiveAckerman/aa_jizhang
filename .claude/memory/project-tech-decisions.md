...
description: 出发AA记账项目的核心技术决策与架构选型
metadata:
  type: project
---

出发AA记账（DepartureAA）多人记账应用的技术决策：
...
- **Logo/品牌素材：** 定稿在 `static/images/logo/`——`logo-icon.png`（纯图标，多人围绕共享钱币）、`logo-card.png`（白色圆角卡片托底版，用于登录页）、`wordmark.png`（艺术字品牌名「出发AA记账」）。登录页用 logo-card + wordmark 组合。全部同步到 miniapp/assets/
