...
- 微信登录与个人资料
- 账本 CRUD、自定义分组、搜索、邀请、加入、退出、复制和成员移除
- 共享账/私密账、四种分账算法（含逐成员输入）、图片凭证、位置和汇率换算
- OCR 识别付款截图自动记账、账单内嵌"我应付"金额
- 智能结算（最优转账方案、单条/批量结算）、交易修改日志和基础统计

当前尚未完成成员排行、结算状态汇总、Excel 导出/备份/恢复、邮箱登录及应用设置。详细状态见 [产品设计文档](./docs/product-design-doc.md)。
