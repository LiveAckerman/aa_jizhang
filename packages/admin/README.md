...
当前开放三个资源：用户、账本和账单。用户只允许编辑昵称和头像；账本支持新建、编辑、删除；账单支持新建、编辑、删除。批量删除仍关闭，避免一次操作跨越多条业务记录。账单写入仍会执行成员校验、结算锁定、分账重算和原有账单日志。后台写操作另外写入 `admin_audit_logs`。`openid`、`unionid`、`inviteCode` 会从 AdminJS API 响应中移除，且不能用于筛选、排序或搜索。账单的 CNY `amount` 在列表和详情按元展示，分账明细和关联账单金额也按元展示；数据库及写入接口继续沿用现有金额单位约定。
